...
import numpy as np
import torch
from src.utils2 import setupt_logger, set_random_seeds, Checker
...
class Trainer(embedder):
    ...
    def __init__(self, args, logger=None):
        ...
        if args.pretrain_embedding_epoch is None:
            self.logger.info("===== Training embedding from scratch =====")
        else:
    ...
    def train(self):
        ...
        valid_dataset = ValidData(self.args, self.item_context, self.train_data, self.test_data, self.valid_data,
                                  self.inference_negative_sampler)
        self.valid_loader = data_utils.DataLoader(valid_dataset, batch_size=self.args.batch_size, shuffle=True,
                                                  drop_last=False)

        test_dataset = TestData(self.args, self.item_context, self.train_data, self.test_data, self.valid_data,
                                self.inference_negative_sampler)
        ...
        params_config = [
            {"params": (p for n, p in self.model.named_parameters() if 'emb' not in n)},
            {"params": (p for n, p in self.model.named_parameters() if 'emb' in n), "lr": 0.1 * self.args.lr}
        ]
        # adam_optimizer = torch.optim.Adam(params_config, lr=self.args.lr, betas=(0.9, 0.98))
        adam_optimizer = torch.optim.Adam(self.model.parameters(), lr=self.args.lr, betas=(0.9, 0.98))
        for epoch in tqdm(range(1, self.args.e_max + 1)):
            self.model.train()
            training_loss = 0.0
            emb_grad_L2 = [0.0, 0.0]    # [item_emb_grad_L2, pos_emb_grad_L2]
            emb_grad_L1 = [0.0, 0.0]    # [item_emb_grad_L1, pos_emb_grad_L1]
            for _, (u, seq, pos, neg) in enumerate(train_loader):
                adam_optimizer.zero_grad()
                u, seq, pos, neg = np.array(u), np.array(seq), np.array(pos), np.array(neg)
                pos_logits, neg_logits = self.model(u, seq, pos, neg)
                pos_labels, neg_labels = torch.ones(pos_logits.shape).to(self.device), torch.zeros(neg_logits.shape).to(
                    self.device)
                ...
                for embedding_layer in [self.model.item_emb, self.model.pos_emb]:
                    emb_grad_L2_layer = 0.0
                    emb_grad_L1_layer = 0.0
                    for param in embedding_layer.parameters():
                        if param.grad is not None:
                            emb_grad_L2_layer += torch.norm(param.grad, 2).item()
                            emb_grad_L1_layer += torch.norm(param.grad, 1).item()
                    if embedding_layer == self.model.item_emb:
                        emb_grad_L2[0] += emb_grad_L2_layer
                        emb_grad_L1[0] += emb_grad_L1_layer
                    else:
                        emb_grad_L2[1] += emb_grad_L2_layer
                        emb_grad_L1[1] += emb_grad_L1_layer

            if epoch % 1 == 0:
                pass
                # print(
                    # f'Epoch: {epoch}, Evaluating: Dataset({self.args.dataset}), Model: ({self.args.model}), Training Loss: {training_loss:.3f}')
                

            if epoch % 5 == 0:
                self.model.eval()
                result_valid = self.evaluate(self.model, k=10, is_valid='valid')
                best_valid = self.validcheck(result_valid, epoch, self.model,
                                             f'{self.args.model}_{self.args.dataset}.pth')
                print(f'Epoch: {epoch}, Valid HIT@10: {result_valid["HIT"]:.4f}, NDCG@10: {result_valid["NDCG"]:.4f}')
